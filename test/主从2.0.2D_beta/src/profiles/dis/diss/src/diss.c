...
const struct attm_desc diss_att_db[DIS_IDX_NB] =
{
    ...
    // PnP ID Characteristic Declaration
    [DIS_IDX_SENSOR_LOC_CHAR]                   =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // PnP ID Characteristic Value
    [DIS_IDX_SENSOR_LOC_VAL]                    =   {ATT_CHAR_SENSOR_LOC, PERM(RD, ENABLE), PERM(RI, ENABLE), DIS_PNP_ID_LEN},
		
		 // PnP ID Characteristic Declaration
    [DIS_IDX_GLUCOSE_FEATURE_CHAR]                   =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // PnP ID Characteristic Value 
    [DIS_IDX_GLUCOSE_FEATURE_VAL]                    =   {ATT_CHAR_GLUCOSE_FEATURE, PERM(RD, ENABLE), PERM(RI, ENABLE), DIS_PNP_ID_LEN},

     // PnP ID Characteristic Declaration
    [DIS_IDX_NEW_ALERT_CHAR]                   =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // PnP ID Characteristic Value 
    [DIS_IDX_NEW_ALERT_VAL]                    =   {ATT_CHAR_NEW_ALERT, PERM(RD, ENABLE), PERM(RI, ENABLE), DIS_PNP_ID_LEN},
...
static uint8_t diss_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct diss_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        env->id                     = TASK_ID_DISS;
        ///env->role                   = PRF_SERVICE; //////discovery ios ancs serivce error
