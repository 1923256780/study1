...
extern uint16_t HB_FFF2_Handle_val,HB_FFF1_Handle_val,HB_Read2A29_Handle_val,HB_Read2A24_Handle_val,HB_Read2A2A_Handle_val,HB_Read2ACC_Handle_val,HB_Read2A37_Handle_val;
#if USB_DRIVER
                                  ...
                                  ke_task_id_t const src_id)
{
   ...
   // uart_printf("offset = 0x%x\r\n",param->offset);
    uart_printf("value %s\n ",param->value);
    GetReadValData(param->length,param->value);
                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    if(src_id == Heart_SRC_ID){
        
        if(param->handle == HB_Read2A37_Handle_val){
            
            fitshow_protocol_forHeartSlave((uint8_t *)param->value,param->length);
        }else{
            
        }
    }else if(src_id == BOX_SRC_ID){

        
        if(param->handle == HB_FFF1_Handle_val){
            
            fitshow_protocol_forBoxSlave((uint8_t *)param->value,param->length);
        }else{
