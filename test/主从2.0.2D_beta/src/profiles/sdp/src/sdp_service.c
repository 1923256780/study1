...
uint16_t HB_FFF2_Handle_val = 0,HB_FFF1_Handle_val = 0,HB_Read2A29_Handle_val = 0,HB_Read2A24_Handle_val = 0,HB_Read2A2A_Handle_val = 0,HB_Read2ACC_Handle_val = 0,HB_Read2A37_Handle_val = 0;
...
uint8_t sdp_extract_svc_info(uint8_t conidx,struct gattc_sdp_svc_ind const *ind,uint16_t hb_src_id)
{
                ...
                if((val_hdl <= ind->end_hdl) && (val_hdl > (ind->start_hdl + fnd_att)))
                {
                    ...
                    for (svc_char=0; svc_char < ind->end_hdl -ind->start_hdl ; svc_char++)
                    {
                      ...
                      //  UART_PRINTF("char uuid = 0x%04x \r\n",attm_convert_to16((uint8_t *)ind->info[val_idx].att.uuid,ind->info[val_idx].att.uuid_len));
                        // if(ind->info[val_idx].att.uuid_len>2){
                            ...
                            for(int i = 0; i < ind->info[val_idx].att.uuid_len; i++)
                            {
                                ...
                                HB_cmpData[i] = ind->info[val_idx].att.uuid[ind->info[val_idx].att.uuid_len - i - 1];
                                if(i>=9)break;
                        ...
                        if(hb_src_id == BOX_SRC_ID)
                        {
                            if(HB_cmpData[0] == 0xFF && HB_cmpData[1] == 0xF2 && ind->uuid[1] == 0xFF && ind->uuid[0] == 0xF0)
                            {
                             
                                HB_FFF2_Handle_val = char_hdl;
                            }
                            else if(HB_cmpData[0] == 0xFF && HB_cmpData[1] == 0xF1 && ind->uuid[1] == 0xFF && ind->uuid[0] == 0xF0)
                            {
                            
                                HB_FFF1_Handle_val = val_hdl;//
                                
                            }
                            else if(HB_cmpData[0] == 0x2A && HB_cmpData[1] == 0x29 && ind->uuid[1] == 0x18 && ind->uuid[0] == 0x0A)
                            {
                              
                                HB_Read2A29_Handle_val = val_hdl;//
                                
                            }
                            else if(HB_cmpData[0] == 0x2A && HB_cmpData[1] == 0x24 && ind->uuid[1] == 0x18 && ind->uuid[0] == 0x0A)
                            {
                       
                                HB_Read2A24_Handle_val = val_hdl;
                            }
                            else if(HB_cmpData[0] == 0x2A && HB_cmpData[1] == 0x2A && ind->uuid[1] == 0x18 && ind->uuid[0] == 0x0A)
                            {
                       
                                HB_Read2A2A_Handle_val = val_hdl;
                            }
                            else if(HB_cmpData[0] == 0x2A && HB_cmpData[1] == 0xCC && ind->uuid[1] == 0x18 && ind->uuid[0] == 0x26)
                            {
                            
                                HB_Read2ACC_Handle_val = val_hdl;
                            }
                            else{
                                
                            }
                        }else if(hb_src_id == Heart_SRC_ID){
                            if(HB_cmpData[0] == 0x2A && HB_cmpData[1] == 0x37 && ind->uuid[1] == 0x18 && ind->uuid[0] == 0x0D)
                            {
                               
                                HB_Read2A37_Handle_val = val_hdl;
                            }
                            else{
