...
static int fff0s_fff4_val_upd_req_handler(ke_msg_id_t const msgid,
                                            struct fff0s_fff45_val_upd_req const *param,
                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    if(state == FFF0S_IDLE)
    {
        ...
        if(fff0s_env->ntf_cfg[conidx] == PRF_CLI_START_NTF)
        {
            ...
            fff0s_notify_fff4_val(conidx,fff0s_env, param);
...
// static int fff0s_fff5_val_upd_req_handler(ke_msg_id_t const msgid,
//                                             struct fff0s_fff45_val_upd_req const *param,
//                                             ke_task_id_t const dest_id,
//                                             ke_task_id_t const src_id)
// {
...
//             // put task in a busy state
//             ke_state_set(dest_id, FFF0S_BUSY);						
//             fff0s_indicate_fff5_val(conidx,fff0s_env, param);
//             //ke_state_set(dest_id, FFF0S_IDLE);
//         }
        ...
        ke_task_id_t const src_id)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
		...
		// else if(att_idx == FFF0S_IDX_FFF5_VAL_IND_CFG)
		// {
		// 	// CCC attribute length = 2
        //     cfm->length = 2;
		// }
        // else if(att_idx == FFF0S_IDX_FFF3_VAL_VAL)
        // {
        //     cfm->length = FFF0_CHAR_DATA_LEN;
        // }
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    // retrieve handle information
    uint8_t status = fff0s_get_att_idx(param->handle,  &att_idx);
    // If the attribute has bffn found, status is GAP_ERR_NO_ERROR
    if (status == GAP_ERR_NO_ERROR)
    {
        struct fff0s_env_tag* fff0s_env = PRF_ENV_GET(FFF0S, fff0s);
                ...
                && ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF)))
        {
            ...
            if (ntf_cfg == PRF_CLI_START_NTF)
            {
                ...
                fff0s_env->ntf_cfg[conidx] = PRF_CLI_START_NTF ;
            }
            else
            {
                // Ntf cfg bit set to 0
                fff0s_env->ntf_cfg[conidx] = PRF_CLI_STOP_NTFIND;
            }

            // Inform APP of configuration change
            struct fff0s_fff4_val_ntf_cfg_ind * ind = KE_MSG_ALLOC(FFF0S_FFF4_VALUE_NTF_CFG_IND,
                    prf_dst_task_get(&(fff0s_env->prf_env), conidx), dest_id,
                    fff0s_fff4_val_ntf_cfg_ind);
            ind->conidx = conidx;
        ...
        //     {
        //         // Ntf cfg bit set to 1
        //         fff0s_env->ind_cfg[conidx] = PRF_CLI_START_IND;
        //     }
        //     else
        //     {
        //         // Ntf cfg bit set to 0
        //         fff0s_env->ind_cfg[conidx] = PRF_CLI_STOP_NTFIND;
        //     }

        //     // Inform APP of configuration change
        //     struct fff0s_fff5_val_ind_cfg_ind * ind = KE_MSG_ALLOC(FFF0S_FFF5_VALUE_IND_CFG_IND,
        //             prf_dst_task_get(&(fff0s_env->prf_env), conidx), dest_id,
        //             fff0s_fff5_val_ind_cfg_ind);
        //     ind->conidx = conidx;
        //     ind->ind_cfg = fff0s_env->ind_cfg[conidx];
						
        //     ke_msg_send(ind);			
        // }
		else if (att_idx == FFF0S_IDX_FFF2_VAL_VAL)
		{
			// Allocate the alert value change indication
			struct fff0s_fff23_writer_ind *ind = KE_MSG_ALLOC(FFF0S_FFF2_WRITER_CMD_IND,
			        prf_dst_task_get(&(fff0s_env->prf_env), conidx),
			        dest_id, fff0s_fff23_writer_ind);
		...
		// {
		// 	// Allocate the alert value change indication
		// 	struct fff0s_fff23_writer_ind *ind = KE_MSG_ALLOC(FFF0S_FFF3_WRITER_REQ_IND,
		// 	        prf_dst_task_get(&(fff0s_env->prf_env), conidx),
		// 	        dest_id, fff0s_fff23_writer_ind);
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        // if (att_idx == FFF0S_IDX_FFF1_VAL_VAL)
        // {
        //     length = 10 * sizeof(uint8_t);
        // }
        // else if (att_idx == FFF0S_IDX_FFF1_USER_DESC)
        // {
        //     length = fff0s_env->fff1_desc_len;
        //     uart_printf("read FFF1_USER_DESC\r\n");
        // }
        // // read notification information
        // else 
        if (att_idx == FFF0S_IDX_FFF4_VAL_NTF_CFG)
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        //     memcpy(cfm->value,fff0s_env->fff1_val,10);
        //     fff0s_env->fff1_val[0]++;
        //     fff0s_env->fff1_val[9]++;
        // }
        // else if (att_idx == FFF0S_IDX_FFF1_USER_DESC)
        // {
        //     memcpy(cfm->value,fff0s_env->fff1_desc,length);
        ...
        if (att_idx == FFF0S_IDX_FFF4_VAL_NTF_CFG)
        {
            uint16_t ntf_cfg = fff0s_env->ntf_cfg[conidx];
            co_write16p(cfm->value, ntf_cfg);
        }  
		
		// else if(att_idx == FFF0S_IDX_FFF5_VAL_IND_CFG)
		// {
		// 	uint16_t ind_cfg = fff0s_env->ind_cfg[conidx];
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    {	           
        struct fff0s_fff45_val_upd_rsp *rsp = KE_MSG_ALLOC(FFF0S_FFF4_VALUE_UPD_RSP,
                                             prf_dst_task_get(&(fff0s_env->prf_env), conidx),
                                             dest_id, fff0s_fff45_val_upd_rsp);
        rsp->conidx = conidx;
    ...
    // {
    //     struct fff0s_fff45_val_upd_rsp *rsp = KE_MSG_ALLOC(FFF0S_FFF5_VALUE_UPD_RSP,
    //                                          prf_dst_task_get(&(fff0s_env->prf_env), conidx),
    //                                          dest_id, fff0s_fff45_val_upd_rsp);
    //     rsp->conidx = conidx;
    //     rsp->status = param->status;			
...
{
    {FFF0S_FFF4_VALUE_UPD_REQ,      (ke_msg_func_t) fff0s_fff4_val_upd_req_handler},
	// {FFF0S_FFF5_VALUE_UPD_REQ,      (ke_msg_func_t) fff0s_fff5_val_upd_req_handler},
