...
void vFitShowDeepSleepWakeupSet(uint8_t gpio, Pull_Type Pull, int_mode mode)
{
    //setf_PMU_Reg0x5_deep_wake_by_gpio;
    gpio_config(gpio, INPUT, Pull);
    addPMU_Reg0x3 |= 1<<((gpio>>4)*8+(gpio&0x0f));	// enable wake-up

    if(((gpio>>4)*8+(gpio&0x0f))<16)
         addAON_GPIO_Reg0x30 |= (mode<<(((gpio>>4)*8+(gpio&0x0f))*2));	
    else
         addAON_GPIO_Reg0x31 |= (mode<<(((gpio>>4)*8+(gpio&0x0f)-16)*2));	
