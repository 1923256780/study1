...
/**
 * @brief 向非易失性数据存储（NVDS）中写入数据。
 *
 * 此函数将数据写入NVDS，用于存储配置参数、用户设置和其他非易失性数据。
 *
 * @param tag 数据的标签标识符。
 * @param length 要写入的数据长度。
 * @param buf 指向包含要写入的数据的缓冲区。
 *
 * @return 如果操作成功，则返回NVDS_OK。
 *         如果指定的参数被锁定，则返回NVDS_PARAM_LOCKED。
 *         如果NVDS中没有足够的空间，则返回NVDS_NO_SPACE_AVAILABLE。
 *
 * 函数首先解析NVDS中的所有现有TAG元素，以便：
 *   1) 查找任何与给定标签匹配的现有标签并擦除它。
 *   2) 计算所有有效标签所需的总长度。
 *   3) 检索可存储新数据的第一个地址。
 *
 * 然后，它将数据写入NVDS，并相应地更新标签头。
 */
